...
import { Printer, UserCheck as UserSearch, DollarSign, Hash, ArrowLeft, Filter, RotateCcw, Clock } from 'lucide-react';
      ...
      <Card className="glass-effect border-amber-800/50">
        <CardHeader>
          <div className="flex items-center">
            <UserSearch className="w-6 h-6 mr-3 text-amber-400" />
            <CardTitle className="text-amber-100">Cashier Transaction Report</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          <div className="max-w-md">
            <Select onValueChange={setSelectedCashier} value={selectedCashier || ''}>
              <SelectTrigger className="bg-black/20 border-amber-800/50 text-amber-100">
                <SelectValue placeholder="Select a cashier to view transactions..." />
              </SelectTrigger>
              <SelectContent>
                {staff.map(member => (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="glass-effect border-green-800/50">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-400">UGX {cashierStats.totalSales.toLocaleString()}</div>
              </CardContent>
            </Card>
          ...
          <Card className="glass-effect border-amber-800/50">
            <CardHeader>
              <CardTitle className="text-amber-100">Transaction History for {selectedCashier}</CardTitle>
            </CardHeader>
            <CardContent>
                    ...
                    {cashierSales.length > 0 ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={6} className="text-center text-amber-200/60 h-24">
                          No transactions found for this cashier in the selected time frame.
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
